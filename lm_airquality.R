...
airquality$Ozone[which(is.na(airquality$Ozone))]<-mean(airquality$Ozone,na.rm = T)
airquality$Solar.R[which(is.na(airquality$Solar.R))]<-mean(airquality$Solar.R,na.rm = T)
...
plot(airquality$Ozone,airquality$Solar.R) # scatter plot between X and Y
...
p1<- predict(model1,data.frame("Solar.R"=10))
p1
